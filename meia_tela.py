...
class MeiaTela(Screen):
    def __init__(self, **kwargs):
        ...
        self.title_image = Image(
            source="Bonecos/titulo_operacoes.png",
            size_hint=(None, None),
            height=dp(80),
            width=dp(300),
            allow_stretch=True,
        ...
        boneco = Image(
            source="Bonecos/boneco_operacao.png",
            size_hint=(0.45, 0.50),
...
class TelaRepresentacoes(Screen):
    def __init__(self, **kwargs):
        ...
        # 2. Cabeçalho (Seta Voltar + Título)
        # Seta Voltar (Preta)
        ...
        # 3. CARD DO RESULTADO (Onde aparece a conta)
        card_resultado = MDCard(
            size_hint=(0.85, 0.25),
            pos_hint={"center_x": 0.5, "top": 0.82},
            md_bg_color=(1, 1, 1, 0.9), # Branco quase sólido
            radius=[20],
            elevation=4,
            orientation="vertical",
            padding=dp(10)
        ...
        # Label principal da conta (Ex: 5 + 3 = 8)
        ...
        # Label secundário (para Resto ou nomes técnicos)
        self.info_label = MDLabel(
            text="Parcela + Parcela = Soma",
            halign="center",
            theme_text_color="Custom",
            text_color=(0.5, 0.5, 0.5, 1), # Cinza
            font_style="Subtitle1"
        )
        ...
        # 5. BOTÕES DE OPERAÇÃO (Ficam fixos embaixo)
        ops_box = BoxLayout(
        ...
        self.botoes_op = {} # Dicionário para guardar os botões e mudar a cor depois
        ...
        for simbolo, icone, id_op in operacoes:
            btn = MDIconButton(
                ...
                theme_text_color="Custom",
                text_color=(1, 1, 1, 1), # Ícone branco
                md_bg_color=(0.2, 0.4, 0.9, 1), # Azul padrão
                icon_size="32sp",
                size_hint=(None, None),
                size=(dp(56), dp(56)),
                pos_hint={"center_y": 0.5}
    ...
    def atualizar_visual_botoes(self):
        """Deixa o botão selecionado Laranja e os outros Azuis"""
        cor_ativa = (1, 0.5, 0, 1) # Laranja
        cor_inativa = (0.2, 0.4, 0.9, 1) # Azul

        for op, btn in self.botoes_op.items():
            if op == self.op_atual:
                btn.md_bg_color = cor_ativa
                # Animaçãozinha simples de escala (opcional)
                # btn.font_size = "40sp"
            else:
    ...
    def atualizar_calculo(self, *args):
        ...
        if self.op_atual == "soma":
            ...
            # Garante que o primeiro número seja maior para não dar negativo
            maior = max(val1, val2)
            menor = min(val1, val2)

            self.label_s1.text = f"Minuendo: {maior}"
            self.label_s2.text = f"Subtraendo: {menor}"

            res = maior - menor
            self.conta_label.text = f"{maior} - {menor} = {res}"
            self.info_label.text = "Resto (Diferença)"
            ...
            # Evita divisão por zero e números feios
            ...
            self.conta_label.text = f"{dividendo} ÷ {divisor} = {quociente}"

            if resto == 0:
                self.info_label.text = "Divisão Exata (Resto 0)"
            else:
                self.info_label.text = f"Divisão Não Exata (Resto {resto})"

    def voltar(self, *args):
        self.manager.transition = SlideTransition(direction="right", duration=0.4)
        self.manager.current = "tela" # Nome da sua tela de menu de operações
