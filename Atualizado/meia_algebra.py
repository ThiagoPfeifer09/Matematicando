...
class AlgebraRepresentacoes(Screen):
    def __init__(self, **kwargs):
        ...
        # 2. Cabeçalho (Alinhado bem no topo)
        # Botão Voltar
        ...
        # 4. GRÁFICO MATPLOTLIB (Com fontes aumentadas)
        ...
        # Botões Extras (Preto e com borda)
    ...
    def gerar_passo_a_passo(self, a, b, c):
        ...
        if self.tipo_atual == "1grau":
            ...
            if a == 0:
                texto += "Como a = 0, a função é constante (uma linha reta horizontal).\nNão toca o eixo X (a menos que b=0)."
            else:
                raiz = -b/a
        ...
        # --- Lógica do 2º Grau (Completa) ---
        else:
            # PROTEÇÃO CONTRA ZERO: Se a=0, não é 2º grau
            if a == 0:
                return "[b]Atenção:[/b] Para ser uma função de 2º grau, o valor de 'a' não pode ser zero.\n\nSe a=0, ela se torna uma função de 1º grau (uma reta).\nMude o valor de 'a' para ver a parábola."

            # 1. Delta
            delta = b**2 - 4*a*c
            texto += f"[b]1. Cálculo do Delta (Δ):[/b]\n"
            texto += f"   Δ = b² - 4.a.c\n"
            texto += f"   Δ = ({b:.2f})² - 4 . ({a:.2f}) . ({c:.2f})\n"
            texto += f"   Δ = {b**2:.2f} - ({4*a*c:.2f})\n"
            texto += f"   [b]Δ = {delta:.2f}[/b]\n\n"

            # 2. Bhaskara
            texto += f"[b]2. Fórmula de Bhaskara:[/b]\n"
            texto += f"   x = (-b ± √Δ) / 2a\n"
            ...
            if delta >= 0:
                ...
                texto += f"   x = (-({b:.2f}) ± {raiz_delta:.2f}) / {2*a:.2f}\n"
                texto += f"   [b]x'[/b] = ({-b:.2f} + {raiz_delta:.2f}) / {2*a:.2f} = [b]{x1:.2f}[/b]\n"
                texto += f"   [b]x''[/b] = ({-b:.2f} - {raiz_delta:.2f}) / {2*a:.2f} = [b]{x2:.2f}[/b]\n\n"
            else:
            ...
            xv = -b / (2*a)
            yv = -delta / (4*a)
            texto += f"[b]3. Vértice (Ponto de Curva):[/b]\n"
            texto += f"   Xv = -b/2a = -({b:.2f})/{2*a:.2f} = [b]{xv:.2f}[/b]\n"
            texto += f"   Yv = -Δ/4a = -({delta:.2f})/{4*a:.2f} = [b]{yv:.2f}[/b]"
    ...
    def atualizar_grafico(self, *args):
        ...
        # Configurações Visuais Otimizadas para Leitura
