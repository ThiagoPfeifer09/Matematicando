...
class TelaFracoesPropriedades(Screen, ThemeManagerMixin):
    def __init__(self, **kwargs):
        ...
        content_box.add_widget(
            MDLabel(
                text=" DEFINIÇÕES: A ESTRUTURA DA FRACÃO",
                halign="center",
                font_style="H5",
                size_hint_y=None,
                height=dp(30),
...
class TelaFracoesInfo(Screen, ThemeManagerMixin):
    VIDEO_SOURCE = "gift.mp4"

    def __init__(self, **kwargs):
        super().__init__(**kwargs)

        # 1. Definir um padrão inicial
        ...
        # Certifique-se que o destino "fracoes" é o name="" da sua FracoesGameScreen no main.py
        topics = [
        ...
        for text, destino in topics:
            btn = MDRaisedButton(
                ...
                size_hint_y=None,
                height="80dp",
                # Cor diferente para o botão de jogar
    ...
    # --- 2. Método para RECEBER a dificuldade da TelaJogar ---
    def definir_dificuldade(self, dificuldade):
        self.dificuldade = dificuldade
        print(f"[TelaFracoesInfo] Dificuldade recebida e armazenada: {self.dificuldade}")

    def on_enter(self, *args):
        if self.video_widget and self.video_widget.source:
            try:
                self.video_widget.state = 'play'
                self.video_widget.seek(0)
            except Exception as e:
                print(f"Erro ao tentar iniciar/reiniciar o vídeo: {e}")
    ...
    def navegar_para_destino(self, destino_tela, *args):
        ...
        # --- 3. Lógica para REPASSAR a dificuldade para o Jogo ---
        if destino_tela == "fracoes": # Se o usuário clicou em JOGAR
            try:
                # Pega a tela do jogo real
                game_screen = self.manager.get_screen("fracoes")

                # Se a tela do jogo tiver o método, injeta a dificuldade que guardamos aqui
                if hasattr(game_screen, 'definir_dificuldade'):
                    print(f"[TelaFracoesInfo] Passando dificuldade {self.dificuldade} para o Jogo")
                    game_screen.definir_dificuldade(self.dificuldade)
            except Exception as e:
                print(f"Erro ao passar dificuldade para o jogo: {e}")
    ...
    def voltar(self, instance):
        self.on_leave()
        self.manager.transition = SlideTransition(direction="right", duration=0.4)
        self.manager.current = "jogar" # Volta para o menu de seleção



    # --- WIDGETS DE FRAÇÃO (MANTIDOS) ---
