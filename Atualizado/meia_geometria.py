...
class GeometriaRepresentacoes(Screen):
    def __init__(self, **kwargs):
        ...
        self.title_label = Label(
            text="GEOMETRIA", color=(0, 0, 0, 1), font_name="BungeeShade",
            font_size="28sp", size_hint=(1, None), height=dp(60),
            pos_hint={"center_x": 0.5, "top": 0.96},
        )
        layout.add_widget(self.title_label)

        # 3. CARD DE INFORMAÇÕES (Expandido para caber mais conta)
        self.info_card = MDCard(
            size_hint=(0.95, 0.28), # Aumentado largura e altura levemente
        ...
        self.lbl_passos = MDLabel(
            ...
            text_color=(0.1, 0.1, 0.1, 1), font_style="Body1", # Fonte maior no texto explicativo
        ...
        # 4. ÁREA DE DESENHO (Menor card, desenho maior dentro)
        self.drawing_container = MDCard(
            size_hint=(0.9, 0.28), # Reduzido altura do card (era 0.35)
        ...
        for nome, icone, id_forma in formas:
            btn = MDIconButton(
                ...
                md_bg_color=(0.2, 0.4, 0.9, 1), icon_size="28sp", # Ícones um pouco menores pra caber
        ...
        btn_trap = MDIconButton(
            icon="shape-outline", theme_text_color="Custom", text_color=(1, 1, 1, 1),
    ...
    def selecionar_forma(self, forma):
        ...
        # Ajustei os valores MAX para serem menores, assim o desenho multiplicado fica grande
        configs = {
            "circle": [("Raio", 10, 50, 30)], # Max reduzido de 80 para 50
            "square": [("Lado", 20, 80, 50)], # Max reduzido
    ...
    def desenhar_texto_canvas(self, texto, x, y):
        # AUMENTO DA FONTE DO LABEL DA FIGURA
        label = CoreLabel(text=texto, font_size=20, color=(0, 0, 0, 1), bold=True) # Fonte 20 e Negrito
        label.refresh()
        texture = label.texture
        Color(1, 1, 1, 0.6) # Fundo um pouco mais transparente
    ...
    def desenhar_forma(self):
        ...
        # FATOR DE ZOOM (Aumenta o desenho visualmente sem mudar o valor matemático)
        ZOOM = 4.5
        ...
        with self.drawing_area.canvas:
            ...
            # Line width aumentado para ficar mais visível

            if self.forma_atual == "circle":
                r = vals["Raio"]
                r_vis = r * ZOOM # Raio Visual
                ...
                self.desenhar_texto_canvas(f"L: {fmt(l)}", cx - l_vis/2 - 30, cy) # Texto na lateral
                ...
                self.desenhar_texto_canvas(f"{fmt(b)}", cx, cy - h_vis/2 - 25) # Base embaixo
                self.desenhar_texto_canvas(f"{fmt(h)}", cx + b_vis/2 + 10, cy) # Altura na direita
