...
# ====================== WIDGET: CÉLULA (COM SUPORTE A TEMAS) ======================
...
class CelulaSudoku(ButtonBehavior, MDLabel):
    def __init__(self, row, col, fixed, start_value=0, **kwargs):
        ...
        self.paleta = {
            "Light": {
                "base": (1, 1, 1, 1),
                "fixed": (0.9, 0.9, 0.9, 1),
                "selected": (0.4, 0.7, 0.9, 1),    # Azul Seleção
                "match": (0.6, 0.8, 1.0, 1),       # Azul Match
                "highlight": (0.85, 0.95, 0.85, 1),# Verde Highlight
            ...
            "Dark": {
                "base": (0.18, 0.18, 0.18, 1),     # Cinza Escuro Fundo
                "fixed": (0.25, 0.25, 0.25, 1),    # Cinza Médio (Fixo)
                "selected": (0.1, 0.4, 0.5, 1),    # Teal Escuro (Seleção)
                "match": (0.2, 0.5, 0.7, 1),       # Azul Escuro (Match)
                "highlight": (0.2, 0.3, 0.2, 1),   # Verde Escuro (Highlight)
                "conflict": (0.6, 0.2, 0.2, 1),    # Vermelho Escuro
                "text_fixed": (0.9, 0.9, 0.9, 1),  # Branco Quase Puro
                "text_input": (0.4, 0.8, 1, 1),    # Azul Neon
                "text_error": (1, 0.4, 0.4, 1)     # Vermelho Claro
            }
        }
    ...
    def atualizar_tema(self):
        """Atualiza as cores da célula baseado no tema do App"""
        app = MDApp.get_running_app()
        tema_atual = app.theme_cls.theme_style # "Light" ou "Dark"
        
        # Define a paleta atual
        self.cores_atuais = self.paleta[tema_atual]
        
        # Reaplica o estado visual com as novas cores
...
# ====================== TELA: DEFINIÇÃO DO JOGO (CARD LAYOUT) ======================
...
class TelaDefinicaoSudoku(Screen):
    ...
    def _build_ui(self):
        ...
        # Fundo Base (Sólido, muda com o tema)
        with layout.canvas.before:
            self.layout_bg_color = Color(0.98, 0.98, 0.98, 1) # Default Light
            Rectangle(pos_hint={'x': 0, 'y': 0}, size_hint=(1, 1))

        # --- HEADER ---
        header = MDBoxLayout(orientation='horizontal', size_hint=(1, 0.08), 
                             pos_hint={'top': 1}, padding=dp(10), spacing=dp(10))
        
        btn_voltar = MDIconButton(
            icon="arrow-left",
            theme_text_color="Custom",
            text_color=(0.5, 0.5, 0.5, 1), 
            on_release=self.voltar_jogo
        )
        ...
        self.add_body_text(card, "Se você não tem certeza, não chute! Use a lógica para eliminar possibilidades. Um chute errado pode custar o jogo mais tarde.")
    ...
    def atualizar_tema_conteudo(self):
        ...
        bg_screen = (0.05, 0.05, 0.05, 1) if is_dark else (0.95, 0.95, 0.98, 1)
...
class TelaSudoku(Screen):
    def __init__(self, **kwargs):
        ...
        # Referências de Cores para Troca de Tema
    ...
    def _build_ui(self):
        ...
        # Fundo Dinâmico (Sólido)
        with layout.canvas.before:
            self.layout_bg_color = Color(0.98, 0.98, 0.98, 1) # Default Light
            Rectangle(pos_hint={'x': 0, 'y': 0}, size_hint=(1, 1))
        
        # --- HEADER ---
        header_layout = BoxLayout(orientation='horizontal', size_hint=(0.94, 0.08), 
                                   pos_hint={'center_x': 0.5, 'top': 0.99}, spacing=dp(5))
    ...
    def _criar_barra_inferior(self):
        container = GridLayout(cols=3, size_hint=(1, 0.08), 
    ...
    def _update_bg(self, widget):
        ...
        with widget.canvas.before:
            # Reusa a cor salva ou define padrão se nulo
            ...
            Color(0.8, 0.8, 0.8, 1) # Linha divisória
    ...
    def trocar_tema(self, instance):
        ...
        # 1. Ajuste de cores da UI
        cor_fundo = (0.05, 0.05, 0.05, 1) if is_dark else (0.98, 0.98, 0.98, 1)
    ...
    # ================= AUXILIARES GRÁFICOS E LÓGICA =================
    
    def desenhar_linhas_grade(self, *args):
        grid = self.grade_9x9
        grid.canvas.after.clear()
        
        # Verifica tema para cor das bordas
        app = MDApp.get_running_app()
        is_dark = app.theme_cls.theme_style == "Dark"
        ...
        with grid.canvas.after:
            ...
            Color(1.0, 0.7, 0.0, 1.0) # Amarelo escuro para melhor contraste
    ...
    def _criar_teclado(self):
        ...
        for i in range(1, 6):
            # Removido text_color fixo para adaptar ao tema
    ...
    def _montar_grade(self, dt):
        ...
        # Pega a cor correta dependendo do tema inicial
        app = MDApp.get_running_app()
    ...
    def pausar_jogo(self, instance):
        self.stop_timer()
        # Recria o diálogo para garantir que pegue o tema atual
    ...
    def selecionar_celula(self, row, col):
        ...
        for (r, c), cell in self.cells.items():
            ...
            # Highlight inteligente: destaca números iguais que não são zero e não é a própria célula
    ...
    def inserir_numero(self, num):
        ...
        # Chama a seleção novamente para atualizar os highlights (ex: acender todos os "5")
        self.selecionar_celula(r, c)

    def game_finished(self, win, timeout=False):
        self.stop_timer()
        titulo = "Parabéns!" if win else "Game Over"
        msg = "Você completou o Sudoku!" if win else f"Muitos erros ({self.max_erros})."
        
        self.dialog = MDDialog(title=titulo, text=msg, buttons=[MDFlatButton(text="SAIR", on_release=self.voltar), MDFlatButton(text="NOVO", on_release=self.reiniciar)], auto_dismiss=False)
        self.dialog.open()
    ...
    # --- FUNÇÃO VOLTAR CORRIGIDA (DIRECIONA PARA "jogar") ---
    def voltar(self, *args):
        if self.dialog: self.dialog.dismiss()
        
        if self.manager:
            self.manager.transition = SlideTransition(direction="right")
            # Aponta diretamente para a tela de escolha de jogos
